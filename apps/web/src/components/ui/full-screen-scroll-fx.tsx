    ...
    (
        {
            ...
            reduceMotion,
            smoothScroll: _smoothScroll = false, // enable if you install Lenis
            ...
            return words.map((w, i) => (
                <span className="fx-word-mask" key={i}>
                    <span className="fx-word" ref={(el) => {
                        ...
                        wordRefs.current[sIdx][i] = el as HTMLSpanElement;
                    }}>{w}</span>
        ...
        useLayoutEffect(() => {
            ...
            // initial bg states
            const safeBgs = bgRefs.current.filter(Boolean);
            gsap.set(safeBgs, { opacity: 0, scale: 1.04, yPercent: 0 });
            ...
            // initial description states
            const descs = fixedRef.current?.querySelectorAll('.fx-featured-description');
            descs?.forEach((d, i) => {
                gsap.set(d, {
        ...
        const changeSection = (to: number) => {
            ...
            if (outWords.length) {
                ...
                // simple fade for mixed content/react nodes
                const feat = fixedRef.current?.querySelectorAll('.fx-featured')[from];
                if (feat) gsap.to(feat, { opacity: 0, duration: D * 0.5 });
            }
            ...
            if (inWords.length) {
                ...
                const feat = fixedRef.current?.querySelectorAll('.fx-featured')[to];
                if (feat) gsap.to(feat, { opacity: 1, duration: D });
            }
            ...
            const outDesc = fixedRef.current?.querySelectorAll('.fx-featured-description')[from];
            const inDesc = fixedRef.current?.querySelectorAll('.fx-featured-description')[to];
            if (outDesc) gsap.to(outDesc, { opacity: 0, y: down ? -20 : 20, duration: D * 0.5 });
            if (inDesc) {
                gsap.set(inDesc, { opacity: 0, y: down ? 20 : -20 });
                gsap.to(inDesc, { opacity: 0.6, y: 0, delay: D * 0.4, duration: D * 0.6 });
            }
        ...
        const handleLoadedStagger = () => {
            ...
            // entrance for initial description
            const activeDesc = fixedRef.current?.querySelectorAll('.fx-featured-description')[index];
            if (activeDesc) {
                gsap.fromTo(activeDesc,
                    { opacity: 0, y: 20 },
                    { opacity: 0.6, y: 0, duration: 0.8, delay: 0.6, ease: "power3.out" }
                );
            }
        };
        ...
        return (
            ...
            >
                ...
                <div className="fx-scroll">
                                        ...
                                        {sections.map((s, sIdx) => {
                                            ...
                                            return (
                                                <div key={`C-${s.id ?? sIdx}`} className={`fx-featured ${sIdx === index ? "active" : ""}`}>
                                                    ...
                                                    {s.description && (
                                                        <div className="fx-featured-description">
                                                            {s.description}
                                                        </div>
                                                    )}
                                                </div>
                    ...
                    <div className="fx-end bg-black relative flex flex-col justify-center items-center overflow-hidden">
                        <div className="flex gap-[10vw]">
                            <p className="fx-fin text-white font-black text-[clamp(2rem,8vw,12rem)] tracking-tighter transition-opacity duration-1000">BANKER</p>
                            <p className="fx-fin text-white font-black text-[clamp(2rem,8vw,12rem)] tracking-tighter opacity-50 transition-opacity duration-1000">EXPERT</p>
                        </div>
                    </div>
                </div>
          ...
          .fx-footer-title { color: var(--fx-text); font-size: clamp(1.2rem, 4vw, 3rem); font-weight: 900; letter-spacing: -0.01em; line-height: 0.9; opacity: 0.5; }
          .fx-progress { width: 200px; height: 2px; margin: 1rem auto 0; background: rgba(245,245,245,0.1); position: relative; }
          .fx-progress-fill { position: absolute; inset: 0 auto 0 0; width: 0%; background: var(--fx-text); height: 100%; transition: width 0.3s ease; }
          .fx-progress-numbers { position: absolute; inset: auto 0 100% 0; display: flex; justify-content: space-between; font-size: 0.7rem; color: var(--fx-text); padding-bottom: 8px; font-weight: 700; opacity: 0.5; }

          .fx-end { height: 100vh; display: grid; place-items: center; }
          .fx-fin { transform: rotate(90deg); font-weight: 900; letter-spacing: 0.5em; }
          ...
          @media (max-width: 900px) {
            ...
            .fx-featured-title { font-size: 4rem; }
          }
        `}</style>
            </div>
        );
